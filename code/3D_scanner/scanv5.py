...
####################################################################################################
def oldtonew(curz_points, rotated_points):
    ...
    with open(rotated_points, 'r') as fnew:       #open file to read from 
        ...
        for i in numrows:                           #iterate through each row
            ...
            if flag == 1:                           #if we want to erase the file and write to a fresh one
                with open(curz_points, 'w') as fold:
                    fold.write(i)  #write new point
                    flag = flag + 1                 #update so we append next time
                    fold.close()
            else:
                with open(curz_points, 'a') as fold: #append write
...
###################################################################################################
def record_z_line(z_step, width, step_diz_pin, step_diz_state, step_step_pin, write_flag, file_to_write_to, lidaz_to_origin_dist, z_diz_indicator, tof, commuPin_in, commuPin_out):
    ...
    curz_points_file = file_to_write_to

    #GPIO SETUP
    IO.setmode(IO.BCM)              #declare naming convention for IO pins
    IO.setup(step_diz_pin, IO.OUT)        #set pins as output
    ...
    IO.output(step_diz_pin, step_diz_state)     #set state of dirpin

    if z_diz_indicator == 1: #This means the lidar is going up

        z_location = -width/2 #start from zero and climb

        for i in range(1,width+1):
            
            r = str(z_location) + ','
            theta = '0,'
            z = str(lidaz_to_origin_dist - tof.get_distance()) + '\n'

            if write_flag == 1:
                f = open(curz_points_file, 'w')
                f.write(r+theta+z)
                write_flag = 0
                f.close()
            else:
                f = open(curz_points_file, 'a')
                f.write(r+theta+z)
                f.close()
        ...
        z_location = width/2   #start from top and go down
...
def main():
    ...
    LIDAz_TO_ORIGIN_DIST = 300
    z_STEP = 1                  #200 steps = 1 full rotation
    z_STEP_DIz_PIN = 23
    z_STEP_DIz_STATE = 1
    z_STEP_STEP_PIN = 24
    THETA_STEP_DIz_PIN = 22
    THETA_STEP_DIz_STATE = 1
    ...
    i = 0                           
    z_diz_indicator = 1                       #1 = going up, 0 = going down
    ...
    while int(angle) < FINAL_ANGLE:   #until part has turned an angle of FINAL_ANGLE
        ...
        if write_flag == 1:   #this means overwrite
            record_z_line(z_STEP, width, z_STEP_DIz_PIN, z_STEP_DIz_STATE, z_STEP_STEP_PIN, write_flag, old_file, LIDAz_TO_ORIGIN_DIST, z_diz_indicator, tof, commuPin_in, commuPin_out) 
            write_flag = 0
        else:                 #this means append
            record_z_line(z_STEP, width, z_STEP_DIz_PIN, z_STEP_DIz_STATE, z_STEP_STEP_PIN, write_flag, old_file, LIDAz_TO_ORIGIN_DIST, z_diz_indicator, tof, commuPin_in, commuPin_out)

        rotationStepper(ANGLE_STEP, THETA_STEP_DIz_PIN, THETA_STEP_DIz_STATE, THETA_STEP_STEP_PIN)
        ...
        z_diz_indicator ^= 1                #change direction of z flag with XOR
        z_STEP_DIz_STATE ^= 1     #change motor direction
